...
const TechnoInfo = (props) => {
  return (
    <div className="row">
      ...
      <div className="row">
        <h3 style={{ color: props.color }} className="text-center">
          {props.name}
        </h3>
      </div>
      <Separator width={"50%"} color={props.color} />
      <div className="row">
        <img
          ...
          alt="React Logo non chargé"
        />
        <p className="col-8 m-auto text-justify">{props.text}</p>
      </div>
    </div>
  );
};
  ...
  return (
    <section className="animated-div border border-2 border-black rounded w-75 p-4">
      <TechnoInfo
        ...
        logo={nodeLogo}
        color={"#008000"}
        text={`Dans le paysage dynamique du développement web, choisir la bonne technologie est essentiel pour garantir des performances optimales et une expérience utilisateur exceptionnelle. 
      ...
      <TechnoInfo
        ...
        logo={reactLogo}
        color={"#0dcaf0"}
        text={`Dans le monde numérique d'aujourd'hui, la rapidité, la réactivité et la convivialité sont des éléments essentiels pour capter et retenir l'attention de vos visiteurs. 
